...
def unet(pretrained_weights=None, input_size=(256, 256, 1)):
    ...
    # output_0 = Conv2D(1, (1, 1), name='class0')(conv10)
    # output_1 = Conv2D(1, (1, 1), name='class1')(conv10)

    # model = Model(inputs = inputs, outputs = [output_0, output_1])

    model = Model(inputs=inputs, outputs=conv10)  # JT code

    model.compile(
        optimizer=Adam(lr=1e-4), loss="binary_crossentropy", metrics=["accuracy"]
    )  # BC can add other metrics here
